  ...
  return (
    <div className="reveal">
      <div className="space-y-5 mb-8">
        <p className="text-white2 text-[0.92rem] leading-[1.9]">
      ...
      <div className="grid grid-cols-2 gap-3">
        {highlights.map(({ label, value }, i) => (
          <div
            key={label}
            className={`reveal reveal-delay-${i + 1} p-4 bg-panel border border-[var(--border)] border-l-2 border-l-cyan
              transition-all duration-300 hover:border-l-green hover:bg-panel2 hover:translate-x-1`}
          >
            <h4 className="font-mono text-[0.62rem] text-cyan uppercase tracking-[0.12em] mb-1">{label}</h4>
            <p className="text-white2 text-[0.8rem] whitespace-pre-line">{value}</p>
          </div>
        ))}
      </div>
    </div>
